...
### Agent-Skill Binding
Agents load relevant skills on demand from `.agents/skills/` via the OpenCode skill tool. Plans must include an `## Agents & Skills` table mapping each phase to its agent and activated skills. Wrong assignment = wrong skills loaded at runtime.
...
**New skill**: Create `.agents/skills/{name}/SKILL.md` with YAML frontmatter (`name`, `description`, `user-invocable`, `tier`). Run sync — skill-index.json is regenerated automatically.
...
|--------|---------------------------|-----------|
| Build / Create (frontend) | "add a button", "build a form", "implement this UI", "React component" | `frontend-developer` via OpenCode subagent dispatch |
| Build / Create (backend) | "add an endpoint", "implement this API", "write a migration", "server function" | `backend-developer` via OpenCode subagent dispatch |
| Build / Create (generic/fullstack) | "continue the plan", "implement X" (unclear layer) | `developer` via OpenCode subagent dispatch |
| Fix / Debug | "something is broken", "this crashes", "why does X happen", "it's not working" | `debugger` via OpenCode subagent dispatch |
| Plan / Design | "how should we build X", "let's plan", "what's the approach for" | `planner` via OpenCode subagent dispatch |
| Architecture (backend) | "design the API", "data model", "schema design", "auth strategy" | `backend-architect` via OpenCode subagent dispatch |
| Architecture (frontend) | "routing structure", "component hierarchy", "state management strategy" | `frontend-architect` via OpenCode subagent dispatch |
| Research | "how does X work", "best practices for", "compare A vs B" | `researcher` via OpenCode subagent dispatch |
| Review / Audit | "check my code", "is this good", "review before merge", "audit this" | `code-reviewer` via OpenCode subagent dispatch |
| Security | "security audit", "harden this", "OWASP", "check for vulnerabilities" | `security-auditor` via OpenCode subagent dispatch |
| Infra / CI/CD / Observability | "set up CI/CD", "deploy this", "add monitoring", "Terraform", "Docker" | `devops-engineer` via OpenCode subagent dispatch |
| Test | "add tests", "is this covered", "validate this works" | `tester` via OpenCode subagent dispatch |
| Docs | "document this", "update the docs", "write a spec" | `docs-manager` via OpenCode subagent dispatch |
| Git | "commit", "push", "create a PR", "ship it", "done" | `git-manager` via OpenCode subagent dispatch |
...
**Step 1 — Scan agents**
Read every file in `.opencode/agents/`. For each agent extract the filename-based agent name, the `description` frontmatter, and the prompt body. If the directory is unreadable, **stop and ask the user** before proceeding.
...
**Single intent** → spawn the matched agent directly via OpenCode subagent dispatch.
...
**Auto-trigger rule**: If the user's prompt, the routed intent, an active plan, a handoff chain, or the loaded skill set indicates that one or more specialist agents are relevant, you must trigger those agents automatically from the main conversation. Do **not** require the user to explicitly ask for subagent dispatch.

**Delegation default**: Use `subagent dispatch` automatically when the prompt clearly matches a known agent workflow. Avoid delegation only when the task is trivial, the work is tightly coupled to the current context, or the immediate next step is blocked on local work you should do first.
...
3. **Execute sequentially** — Spawn each agent via OpenCode subagent dispatch one at a time, in order. Wait for the current agent to finish before spawning the next. Pass the previous agent's output as context to the next agent.
