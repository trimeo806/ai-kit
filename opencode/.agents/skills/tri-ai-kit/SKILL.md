...
A multi-agent development toolkit for OpenCode. The main conversation is always the orchestrator — it dispatches specialized agents via OpenCode subagent dispatch and merges their results.
...
Subagents (spawned via OpenCode subagent dispatch) cannot spawn further subagents. All multi-agent orchestration must originate from the main conversation.
