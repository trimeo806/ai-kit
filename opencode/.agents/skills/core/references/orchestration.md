...
```
Main context → [OpenCode subagent dispatch] → brainstormer   (generates 3 independent options)
Main context → [OpenCode subagent dispatch] → researcher     (evaluates options against criteria)
Main context → [OpenCode subagent dispatch] → planner        (selects winner, writes implementation spec)
```
...
Subagents (agents spawned via OpenCode subagent dispatch) **cannot spawn further subagents**. Neither OpenCode subagent dispatch nor Task tool is available in subagent context.
...
**Pattern**:
```
Main context → [OpenCode subagent dispatch] → specialist-1 (independent subagent)
Main context → [OpenCode subagent dispatch] → specialist-2 (independent subagent)
Main context reads both results and merges
```

**Anti-pattern** (will fail):
```
Main context → [OpenCode subagent dispatch] → agent-A (subagent)
                                 → [OpenCode subagent dispatch] → agent-B  ❌ BLOCKED
```
...
|-------------|---------------|
| `context: fork` + `agent: {name}` | **MUST** spawn `{name}` via OpenCode subagent dispatch. Do NOT execute inline. Do NOT use raw Bash. |
...
No "Generated with OpenCode" or AI attribution in commits or PRs.
