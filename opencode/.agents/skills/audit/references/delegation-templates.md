...
**Usage:** These templates are dispatched by the **main conversation** (via `audit/SKILL.md`), NOT by subagents. Subagents cannot spawn further subagents — neither OpenCode subagent dispatch nor Task tool is available in subagent context.

Select the matching template, fill all `{placeholders}`, dispatch via **OpenCode subagent dispatch**, wait for specialist report, then merge findings.
