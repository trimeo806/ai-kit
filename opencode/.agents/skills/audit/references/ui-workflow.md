...
**Delegation intake:** If this workflow was invoked via an OpenCode subagent dispatch delegation (not a direct `/audit --ui` call), read the delegation context block at the start of your task for scope, expectations, output format, and report-back target. Use `scope.file_list` as your file list, `scope.platform` as your platform flag, and send your report to `calling_agent` when done. See `audit/references/delegation-templates.md` for context block examples (Template A, A+, A++).

**Delegation block missing or incomplete?** If invoked via OpenCode subagent dispatch but `Scope:`, `Mode:`, and `Output path:` fields are absent:
...
**Sub-agent** (delegated via OpenCode subagent dispatch):
