...
When invoked via `/git` with no flags or explicit intent, run `git status --short` first, then use `ask the user` with contextual options based on the output. Options must be natural language (no slash commands):
...
- **Git history persists** across OpenCode sessions
...
**CRITICAL**: Confirm before destructive operations using ask the user.
...
**Implementation**:
Use ask the user with options. On "Details", show:
```bash
