...
After generation, **ALWAYS** ask user about HTML preview via `ask the user`. If yes, invoke `/ui-ux-pro-max` for gallery.
...
1. **Gather requirements** via `ask the user` — purpose, platform, content, brand, style, quantity
...
3. **Ideate** — 3-5 concepts, present via `ask the user`
