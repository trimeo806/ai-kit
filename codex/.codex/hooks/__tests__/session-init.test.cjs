 ...
 *
 * Issue #277: Auto-compact bypasses ask the user approval gates
...
describe('session-init.cjs', () => {
  ...
  describe('Issue #277 Mitigation: Compact Warning', () => {
    ...
    it('outputs warning when source=compact', async () => {
      ...
      assert.ok(
        result.stdout.includes('ask the user'),
        'Should mention ask the user'
      );
      assert.ok(
  ...
  describe('Warning Content Validation', () => {
    ...
    it('warning contains all required guidance elements', async () => {
      ...
      assert.ok(
        result.stdout.includes('waiting for user approval via ask the user'),
      ...
      // Line 4: Action
      assert.ok(
        result.stdout.includes('Use ask the user to verify'),
        'Should instruct to use ask the user'
      );
      assert.ok(
  ...
  describe('Issue #291: Git Root Path Resolution', () => {
    ...
    it('shows subdirectory info when CWD differs from git root (Issue #327)', async () => {
      ...
      const subdirPath = require('path').join(gitRoot, '.codex', 'hooks');
