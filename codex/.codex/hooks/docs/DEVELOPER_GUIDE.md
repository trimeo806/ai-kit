...
Hooks trong Codex không phải là AI Agents hay Skills (không dùng Prompt). Chúng là các **Node.js Scripts (`.cjs`) chạy ngầm ở tầng hệ thống (CLI level)**. 
...
Cấu hình quản lý lúc nào kích hoạt hook nào nằm trọn trong file `@[.codex/config.toml and .codex/hooks.json]`.

Thư mục hooks được chia làm 2 phần tĩnh cơ bản:
- `*.cjs`: Các script entrypoint được gọi trực tiếp bởi Codex.
...
1. **Log Crash:** Tất cả lỗi sập của hooks đều được Try-Catch cứng và đẩy log dạng append vào `.codex/hooks/.logs/hook-log.jsonl`. Để xem chi tiết lỗi bạn đọc file này: 
   ```bash
   cat .codex/hooks/.logs/hook-log.jsonl
   ```
