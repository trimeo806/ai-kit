...
metadata:
  ...
  version: "1.2.0"
  domain: language
  triggers: Go, Golang, goroutines, channels, gRPC, microservices Go, Go generics, concurrent programming, Go interfaces, net/http, http.Server, ServeMux, HTTP handler, HTTP middleware, Go HTTP client, REST API Go
  role: specialist
...
Senior Go developer with deep expertise in Go 1.22+, concurrent programming, cloud-native microservices, and idiomatic HTTP servers/clients. Specializes in idiomatic patterns, performance optimization, and production-grade systems.
...
| net/http | `references/net-http.md` | HTTP servers, clients, ServeMux (1.22+), middleware, httptest |
...
Go 1.22+, goroutines, channels, select, sync package, generics, type parameters, constraints, io.Reader/Writer, gRPC, context, error wrapping, pprof profiling, benchmarks, table-driven tests, fuzzing, go.mod, internal packages, functional options, net/http, http.Server, http.ServeMux method+path patterns, r.PathValue, http.Handler, http.HandlerFunc, middleware chaining, http.Client, http.NewRequestWithContext, http.Transport, net/http/httptest, graceful shutdown
