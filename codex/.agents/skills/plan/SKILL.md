...
1. Use the **custom agent dispatch** to spawn `planner`
...
Before generating any phase file, analyze `.codex/agents/` and `.agents/skills/skill-index.json` to assign the right executor to each phase.

### Step A — Scan the Agent Catalog

Read every `*.md` in `.codex/agents/` and extract:
