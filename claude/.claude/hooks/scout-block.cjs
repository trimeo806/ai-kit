 ...
 *
 * Blocks access to directories listed in .claude/.aikit-ignore
 ...
 *
 * Configuration:
 * - Edit .claude/.aikit-ignore to customize blocked patterns (one per line, # for comments)
...
try {
  ...
  const ignoreFileRaw = scoutConfig.ignoreFile || '.claude/.aikit-ignore';
